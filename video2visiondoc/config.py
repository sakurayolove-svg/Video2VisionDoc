"""
config.py —— 统一配置加载（内置默认值 + config.yaml 覆盖）

设计原则：
    - 不读配置文件时，所有阶段的默认值就是推荐配置
      （与实战验证时激活的代码完全一致）；
    - config.yaml 中的每个模块选择项，可把对应阶段切换为同级
      备选模块（复制自 src/ 初版实现）。

模块选择总览（每阶段单开关，同级并列切换）：
...
# 推荐默认值：与实战验证（BV13T3x69Eqz）时激活的配置完全一致
DEFAULT_CONFIG = {
    "bilibili": {
        "method": "api",          # api=API直连（默认）; ytdlp
        "quality": 64,
        "timeout": 300,
        "threads": 4,
        "cookie_file": "",
    ...
    "transcription": {
        "engine": "chunked",      # chunked=分块防OOM（默认）
    ...
    "alignment": {
        "method": "per_slide",    # per_slide=按页（默认）; window=±60s 滑窗
        "window_seconds": 60,
        "exclude_frames": [],     # 剔除帧序号（1 起），如演讲者镜头
    },
    "translation": {
        "engine": "llm",          # llm=按页（默认）; openai/deep-translator/argos=逐段
        "target_language": "中文",
        "preserve_terms": [
    ...
    "vision_doc": {
        "builder": "slide",       # slide=自包含HTML（默认）; legacy=模板生成器
        "output_format": "html",
        "template": "academic",
        "pdf": False,
        # legacy 模块参数（切换后生效）
...
def load_config(config_path: str = None) -> dict:
    """
    加载配置：内置默认值为基础，config.yaml 中的键递归覆盖。
...
def _migrate_legacy_keys(config: dict) -> None:
    """兼容早期版本配置键（mode 双开关 → engine 单开关）"""
    tr = config.get("transcription", {})
    if "mode" in tr:
        mode = tr.pop("mode")
        # 早期版本: mode=chunked + engine=faster-whisper → engine=chunked
    ...
    if "mode" in tl:
        mode = tl.pop("mode")
        # 早期版本: mode=per_page → engine=llm；mode=per_segment → engine 保留原值
        if mode == "per_page":
            tl["engine"] = "llm"
