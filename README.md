...
整个工具是一条六阶段流水线：**下载 → 转写 → 翻译 → 关键帧 → 对齐 → 文档生成**。每个阶段有多个**同级并列**的实现模块，用 config.yaml 中的**一个开关**即可切换（选 whisper 这类细粒度引擎不需要再切别的开关）：

| 阶段 | 开关 | 可选模块（**加粗**为默认，实战验证） |
|------|------|------------------------------------------|
...
# 默认模块的最小依赖：requests / faster-whisper / Pillow / numpy
# 备选模块的追加依赖见 requirements.txt 下半部分
...
**输入**：`config.yaml`（根据你的环境和需求修改；不修改时全部为默认值）

模块切换项（每个阶段独立切换）：

```yaml
bilibili:
  method: "api"              # api(默认,抗反爬) / ytdlp(支持Cookie)

transcription:
  engine: "chunked"          # chunked(默认,分块防OOM)
                             # / faster-whisper / whisper / openai-api(整段)
  model: "small"             # 4GB内存验证值; 有GPU可改 large-v3
  initial_prompt: ""         # 领域提示词, 显著改善专有名词识别

translation:
  engine: "llm"              # llm(默认,按页,OpenAI兼容)
                             # / openai / deep-translator / argos(逐段)
...
vision_doc:
  builder: "slide"           # slide(默认,按页HTML) / legacy(模板)
  pdf: false                 # slide: 同时输出PDF(需playwright或weasyprint)
```

翻译 API（`llm` 引擎，任意 OpenAI 兼容端点）通过环境变量配置：
...
# 切换备选模块示例（命令行覆盖 config.yaml，每阶段一个开关）
...
| 模块 | 文件示例 | 查看方式 |
|------|----------|----------|
| `slide`（默认） | `*_视觉文档.html`（+ 可选 `.pdf`） | 单文件自包含（图片 base64 内嵌），浏览器直接打开，可离线分享 |
| `legacy` | `*_vision_doc.html` / `.md` / `.pdf` | HTML 自包含；Markdown 图片用相对路径便于版本控制 |
...
**`api`（默认）：API 直连**（`video2visiondoc/downloader.py`）
...
**`ytdlp`（备选）：yt-dlp 下载**（`video2visiondoc/downloader_ytdlp.py`，复制自 `src/extractors/bilibili.py`）

- 支持 Cookie 文件（大会员高画质/付费内容）、并发分片、更多画质选择
- **字幕获取**：`x/player/wbi/v2` 获取 B 站人工/AI 字幕（两种下载模块都可用，`--use-subtitle`）

### 3.2 语音转文字

**`chunked`（默认）：分块转写**（`video2visiondoc/transcriber.py`）
...
**`faster-whisper` / `whisper` / `openai-api`（备选）：整段多引擎**（`video2visiondoc/transcriber_standard.py`，复制自 `src/processors/transcriber.py`）
...
**`llm`（默认）：按页翻译**（`video2visiondoc/translator.py`）
...
**`openai` / `deep-translator` / `argos`（备选）：逐段三引擎**（`video2visiondoc/translator_engines.py`，复制自 `src/processors/translator.py`）
...
**`interval_dhash`（默认）：均匀抽帧 + dHash 去重**（`video2visiondoc/keyframes.py`）
...
**`ppt_layout`（备选）：PPT 布局分析**（`video2visiondoc/keyframes_ppt_layout.py`，复制自 `src/extractors/frame_extractor.py`）
...
**`per_slide`（默认）：按 PPT 页时间窗**（`video2visiondoc/aligner.py`）

- 每页 PPT 的停留时间段 `[t_i, t_{i+1})` 内的全部转写段归为该页——翻译粒度与文档结构天然一致
- `exclude_frames` 可剔除演讲者纯镜头帧，其时间窗并入前一页

**`window`（备选）：±60 秒滑窗**（`video2visiondoc/aligner.py` 的 `align_window`）

- 每帧配对前后 60 秒内的字幕，与初版 `VisionDocGenerator` 语义一致

### 3.6 视觉文档生成

**`slide`（默认）：按页自包含 HTML**（`video2visiondoc/docbuilder.py`）

- 每页一节：页码 + 视频时间段 + PPT 截图 + 该页中文译稿
- 图片全部内嵌为 base64 Data URI，单文件离线浏览/邮件/网盘分享
- `--pdf` 时经 Playwright（或 WeasyPrint）输出 A4 PDF

**`legacy`（备选）：模板生成器**（`video2visiondoc/docbuilder_legacy.py`，复制自 `src/generators/vision_doc.py`）
...
| 项目 | 链接 | 用途 |
|------|------|------|
| yt-dlp | https://github.com/yt-dlp/yt-dlp | B 站视频下载（`ytdlp` 模块） |
| faster-whisper | https://github.com/SYSTRAN/faster-whisper | 本地 GPU 加速语音转录 |
| OpenAI Whisper | https://github.com/openai/whisper | 官方语音转录模型（`whisper` 引擎） |
| deep-translator | https://github.com/nidhaloff/deep-translator | 免费翻译引擎（`deep-translator` 引擎） |
...
### BV13T3x69Eqz — Sergei Gukov: 面向长时程稀疏奖励任务的人工智能工具（默认模块组合）
...
- **示例文件**: [`examples/gukov_vision_doc.pdf`](examples/gukov_vision_doc.pdf)（工具直接生成的 PDF，50 帧时间戳对齐版）、[`examples/gukov_vision_doc_27p.pdf`](examples/gukov_vision_doc_27p.pdf)（27 页精排版视觉文档）、[`examples/gukov_vision_doc.html`](examples/gukov_vision_doc.html)、[`examples/example_vision_doc.html`](examples/example_vision_doc.html)
...
**`chunked` 分块转写** (`video2visiondoc/transcriber.py`):
```python
...
### BV13T3x69Eqz — Sergei Gukov（`ppt_layout` 模块）
...
### BV13T3x69Eqz — Sergei Gukov（切片场景变化示例）
